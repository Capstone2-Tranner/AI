...
class PlaceDataCollector:
        ...
        while lat <= max_lat and grid_counter < max_grids:
            ...
            while lng <= max_lng and grid_counter < max_grids:
                ...
                if current_grid_places:
                    self.save_grid_data_local(current_grid_places, grid_counter+1, current_grid_info)
    ...
    def save_grid_data_local(self, places: List[Dict], grid_number: int, grid_info: Dict):
        """
        한 격자점의 모든 데이터를 로컬 디스크에 저장합니다.
        """
        # 로컬 디스크에 JSON 및 전처리 데이터 저장 (LangChain_RAG/data_local)
        base_dir = os.path.dirname(os.path.abspath(__file__))
        local_dir = os.path.join(base_dir, "data_local")
        ...
        raw_path = os.path.join(raw_dir, f"raw_data_{lat_str}_{lng_str}.json")
        with open(raw_path, "w", encoding="utf-8") as f:
            f.write(grid_json)
        logger.info(f"\t로컬에 격자점 ({grid_info['lat']:.3f},{grid_info['lng']:.3f}) JSON 저장 완료: {raw_path}")

        # 전처리된 문장 생성 및 로컬 저장 (preprocessed)
        ...
        sentence_content = "\n\n".join(sentence_lines)
        pre_path = os.path.join(pre_dir, f"preprocessed_data_{lat_str}_{lng_str}.txt")
        with open(pre_path, "w", encoding="utf-8") as f:
            f.write(sentence_content)
        logger.info(f"\t로컬에 전처리된 격자점 ({grid_info['lat']:.3f},{grid_info['lng']:.3f}) 텍스트 저장 완료: {pre_path}")
